						...
						<tr>
							...
							<td>
								@if(!empty($value['Data']))
									...
									<a href="{{ route('file.download.teacher', $value["Data"]) }}" class="btn btn-primary">Download</a> 
								@else
									<button type="submit" class="btn btn-primary">N/A</button>
								@endif
							</td>
							<td>
								@if($value["is_archived"])
								<div class="card-body">
									<form action="{{ route('file.unarchive', $value['Data']) }}" method="GET">
										@csrf
										<button type="submit" class="btn btn-danger">Unarchive</button>
									</form>
								</div>
								@else
                                    <form action="{{ route('file.archive', $value['Data']) }}" method="GET">
										@csrf
